...
const FILE_LINK_TTL_SECONDS = 60 * 60; // 1 hour
...
export async function GET(request, { params }) {
  try {
    ...
    if (payload.moduleId) {
      ...
      if (modError || !mod) {
        return json(
          { error: "This module is no longer available." },
          404
        );
      }
      ...
      if (signError || !signed?.signedUrl) {
        return json(
          { error: `File not found: ${mod.file_path}` },
          500
        );
      }
    ...
    /*
     * REGULAR EBOOK / VIDEO / AUDIO / BONUS
     */
    const filePath = isBonus
    ...
    const title = isBonus
      ? book.bonus_title || `Bonus: ${book.title}`
      : book.title;

    if (!filePath) {
      return json(
        { error: "Nothing to show here." },
        404
      );
    }
    ...
    return json(
      { error: err.message || "Could not load this content." },
      500
    );
  }
}
